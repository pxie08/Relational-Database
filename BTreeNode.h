...
/************************************************************************************/
 	//size of a key (int) + recordid (pid + sid)
	//should be 4+4+4=12 bytes
	//leaf node - size of one element
	static const int ENTRY_SIZE = sizeof(int) + sizeof(RecordId);//12
	//max number of leaf sets one page can hold
	//minus one sizeof(PageID) for the last block in the page that points to next node
	//leaf node - max number of elements in one node/page
	static const int MAX_ENTRY_COUNT = (PageFile::PAGE_SIZE - sizeof(PageId) - sizeof(int) ) / ENTRY_SIZE;//84
	//1024 / 4 = 256
	static const int MAX_TUPLES = PageFile::PAGE_SIZE / sizeof(int);//256
	...
	void writeEntry(RecordId rid, int key, int entryid);
	
	RC setNodeKeyCount(int count);
	
	RC updateNodeKeyCount();
...
/***********************************************************
 * BTNonLeafNode: The class representing a B+tree leaf node
 **********************************************************/

class BTNonLeafNode {
  public:

/***************************Helper Member Functions for NonLeaf**********************/
/************************************************************************************/
	
	static const int ENTRY_SIZE = sizeof(int) + sizeof(PageId);//8
	static const int MAX_ENTRY_COUNT = (PageFile::PAGE_SIZE - sizeof(PageId) - sizeof(int) ) / ENTRY_SIZE - 1;//126
	static const int MAX_TUPLES = PageFile::PAGE_SIZE / sizeof(int);//256
	...
	void writeEntry(int key, PageId pid, int entryid);
	
	RC readEntry(int eid, int& key, PageId& rightPid);
